...
void greentea_memset(const int_tp ctx_id, const uint_tp N, const int_tp alpha,
  ...
  typedef float Dtype;
  viennacl::ocl::kernel &oclk_fill =
      program.get_kernel(CL_KERNEL_SELECT("fillbuffer"));
  viennacl::ocl::enqueue(oclk_fill(static_cast<int_tp>(N),
...
// Copy from main memory to OpenCL buffer
void greentea_gpu_memcpy(const int_tp ctx_id, const uint_tp N, const void *X,
                         cl_mem Y, const int_tp offY) {
  viennacl::ocl::context &ctx = viennacl::ocl::get_context(ctx_id);
  if (X != NULL) {
    clEnqueueWriteBuffer(ctx.get_queue().handle().get(), Y, CL_TRUE, offY, N, X,
                         0, NULL, NULL);
  }
}
...
template <typename Dtype>
void greentea_copy(const int_tp ctx_id, const int_tp N, const cl_mem X,
                   const int_tp offX, Dtype *Y) {
  greentea_gpu_memcpy(ctx_id, sizeof(Dtype) * N, X, offX * sizeof(Dtype), Y);
}

template <typename Dtype>
void greentea_copy(const int_tp ctx_id, const int_tp N, const Dtype *X,
                   cl_mem Y, const int_tp offY) {
  greentea_gpu_memcpy(ctx_id, sizeof(Dtype) * N, X, Y, offY * sizeof(Dtype));
}
...
// Explicit instantiations
template void greentea_copy<int_tp>(const int_tp ctx_id, const int_tp N,
                                    const cl_mem X, const int_tp offX,
                                    int_tp *Y);
template void greentea_copy<uint_tp>(const int_tp ctx_id, const int_tp N,
                                     const cl_mem X, const int_tp offX,
                                     uint_tp *Y);
template void greentea_copy<float>(const int_tp ctx_id, const int_tp N,
                                   const cl_mem X, const int_tp offX, float *Y);
template void greentea_copy<double>(const int_tp ctx_id, const int_tp N,
                                    const cl_mem X, const int_tp offX,
                                    double *Y);

template void greentea_copy<int_tp>(const int_tp ctx_id, const int_tp N,
                                    const int_tp *X, cl_mem Y,
                                    const int_tp offY);

template void greentea_copy<uint_tp>(const int_tp ctx_id, const int_tp N,
                                     const uint_tp *X, cl_mem Y,
                                     const int_tp offY);

template void greentea_copy<float>(const int_tp ctx_id, const int_tp N,
                                   const float *X, cl_mem Y, const int_tp offY);

template void greentea_copy<double>(const int_tp ctx_id, const int_tp N,
                                    const double *X, cl_mem Y,
                                    const int_tp offY);
...
void greentea_gpu_gemm(const int_tp ctx_id, const CBLAS_TRANSPOSE TransA,
  ...
  if (ctx.devices()[0].type() == CL_DEVICE_TYPE_CPU) {
    ...
    typedef typename viennacl::matrix_base<Dtype, uint_tp, int_tp>::size_type
        size_type;
    typedef typename viennacl::matrix_base<Dtype, uint_tp, int_tp>::size_type
        difference_type;
    ...
    viennacl::matrix_base<Dtype, size_t, ptrdiff_t> matA(
        A, ctx, A_size1, size_type(0), difference_type(1), size_type(M),
        A_size2, size_type(offA), difference_type(1),
        size_type(lda) VCL_ROW_MAJOR);

    viennacl::matrix_base<Dtype, size_t, ptrdiff_t> matB(
        B, ctx, B_size1, size_type(0), difference_type(1), size_type(K),
        B_size2, size_type(offB), difference_type(1),
        size_type(ldb) VCL_ROW_MAJOR);

    viennacl::matrix_base<Dtype, size_t, ptrdiff_t> matC(
        C, ctx, size_type(M), size_type(0), difference_type(1), size_type(M),
    ...
    if (std::is_same<Dtype, float>::value) {
      ...
      GREENTEA_CL_BLAS_CHECK(clblasDgemm(
          clOrder, clTransA, clTransB, M, N, K, alpha, A, offA, lda, B, offB,
          ldb, beta, C, offC, ldc, 1, &queue, 0, NULL, NULL));
    }
#endif
...
template void
greentea_gpu_gemm<float>(const int_tp ctx_id, const CBLAS_TRANSPOSE TransA,
                         const CBLAS_TRANSPOSE TransB, const int_tp M,
                         ...
                         const int_tp offC);
template void
greentea_gpu_gemm<double>(const int_tp ctx_id, const CBLAS_TRANSPOSE TransA,
                          const CBLAS_TRANSPOSE TransB, const int_tp M,
                          const int_tp N, const int_tp K, const double alpha,
                          const cl_mem A, const int_tp offA, const cl_mem B,
                          const int_tp offB, const double beta, cl_mem C,
                          const int_tp offC);
...
void greentea_gpu_gemv(const int_tp ctx_id, const CBLAS_TRANSPOSE TransA,
  ...
  if (ctx.devices()[0].type() == CL_DEVICE_TYPE_CPU) {
    ...
    typedef typename viennacl::vector_base<Dtype, uint_tp, int_tp>::size_type
        size_type;
    typedef typename viennacl::vector_base<Dtype, uint_tp, int_tp>::size_type
        difference_type;
    ...
    viennacl::matrix_base<Dtype, size_t, ptrdiff_t> mat(
        A, ctx, size_type(M), size_type(0), difference_type(1), size_type(M),
        size_type(N), size_type(offA), difference_type(1),
        size_type(N) VCL_ROW_MAJOR);
    v2 *= beta;
    ...
    if (std::is_same<Dtype, float>::value) {
      GREENTEA_CL_BLAS_CHECK(clblasSgemv(clblasRowMajor, clTransA, M, N, alpha,
                                         A, offA, N, x, offx, 1, beta, y, offy,
                                         1, 1, &queue, 0, NULL, NULL));
    } else {
      GREENTEA_CL_BLAS_CHECK(clblasDgemv(clblasRowMajor, clTransA, M, N, alpha,
                                         A, offA, N, x, offx, 1, beta, y, offy,
                                         1, 1, &queue, 0, NULL, NULL));
    }
#endif
...
void greentea_gpu_axpy(const int_tp ctx_id, const int_tp N, const Dtype alpha,
  ...
  if (ctx.devices()[0].type() == CL_DEVICE_TYPE_CPU) {
    ...
    viennacl::vector_base<Dtype, size_t, ptrdiff_t> v1(
        X, size_type(N), size_type(offX), difference_type(1), ctx);
    viennacl::vector_base<Dtype, size_t, ptrdiff_t> v2(
        Y, size_type(N), size_type(offY), difference_type(1), ctx);
    ...
    if (std::is_same<Dtype, float>::value) {
      GREENTEA_CL_BLAS_CHECK(clblasSaxpy(N, alpha, X, offX, 1, Y, offY, 1, 1,
                                         &queue, 0, NULL, NULL));
    } else {
      GREENTEA_CL_BLAS_CHECK(clblasDaxpy(N, alpha, X, offX, 1, Y, offY, 1, 1,
                                         &queue, 0, NULL, NULL));
    }
#endif
  }
  ...
  if (ctx.devices()[0].type() == CL_DEVICE_TYPE_CPU) {
    ...
    if (std::is_same<Dtype, float>::value) {
      GREENTEA_CL_BLAS_CHECK(
          clblasSscal(N, alpha, x, offx, 1, 1, &queue, 0, NULL, NULL));
    } else {
      GREENTEA_CL_BLAS_CHECK(
          clblasDscal(N, alpha, x, offx, 1, 1, &queue, 0, NULL, NULL));
    }
#endif
  }
}
...
void greentea_gpu_dot(const int_tp ctx_id, const int_tp n, const cl_mem X,
  ...
  if (ctx.devices()[0].type() == CL_DEVICE_TYPE_CPU) {
    ...
    *out = viennacl::linalg::inner_prod(v1, v2);
    ...
    if (std::is_same<Dtype, float>::value) {
      GREENTEA_CL_BLAS_CHECK(clblasSdot(n, gpuout, 0, X, offX, 1, Y, offY, 1,
                                        scratch, 1, &queue, 0, NULL, NULL));
    } else {
      GREENTEA_CL_BLAS_CHECK(clblasDdot(n, gpuout, 0, X, offX, 1, Y, offY, 1,
                                        scratch, 1, &queue, 0, NULL, NULL));
    }
...
void greentea_gpu_asum(const int_tp ctx_id, const int_tp n, const cl_mem X,
  ...
  if (ctx.devices()[0].type() == CL_DEVICE_TYPE_CPU) {
    ...
    *Y = viennacl::linalg::norm_1(v1);
    ...
    if (std::is_same<Dtype, float>::value) {
      GREENTEA_CL_BLAS_CHECK(clblasSasum(n, gpuout, 0, X, offX, 1, scratch, 1,
                                         &queue, 0, NULL, NULL));
    } else {
      GREENTEA_CL_BLAS_CHECK(clblasDasum(n, gpuout, 0, X, offX, 1, scratch, 1,
                                         &queue, 0, NULL, NULL));
    }
...
void greentea_gpu_set(const int_tp ctx_id, const int_tp N, const Dtype alpha,
  ...
  // OpenCL Version < 1.2 fallback
  viennacl::ocl::kernel &oclk_fill =
      program.get_kernel(CL_KERNEL_SELECT("fill"));
...
void greentea_gpu_rng_uniform(const int_tp ctx_id, const int_tp n, cl_mem r,
  ...
  greentea_gpu_memcpy(ctx_id, sizeof(uint_tp) * n, &random[0], r, offr);
}
...
void greentea_gpu_rng_uniform(const int_tp ctx_id, const int_tp n,
  ...
  greentea_gpu_memcpy(ctx_id, sizeof(Dtype) * n, &random[0], r, offr);
}
