...
int LMDBTransaction::Get(const string& key, vector<char> *value) {
  CHECK(value);
  MDB_val mdb_key, mdb_value;
  mdb_key.mv_data = const_cast<char*>(key.data());
  mdb_key.mv_size = key.size();
  int_tp mdb_status = mdb_get(mdb_txn_, *mdb_dbi_, &mdb_key, &mdb_value);
  if (mdb_status == MDB_SUCCESS) {
    value->resize(mdb_value.mv_size);
    memcpy(value->data(), mdb_value.mv_data, mdb_value.mv_size);
    return 1;
  } else {
    LOG(ERROR) << "Key " << key << "NOT found in lmdb";
    return 0;
  }
}
